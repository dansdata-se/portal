...
There are two variants of text fields: filled, and outlined.
...
<Canvas>
  <Story
    ...
    args={{
      ...
      label: "Label",
      supportingText:
        "This is a very long supporting text that really is very - very - long, eventually creating a need for multiple lines of text that should wrap nicely without overlapping anything at all.",
      characterCount: true,
    }}
  >
    {({
      ...
      return (
        <div className={`p-6 pb-16 bg-${parentBackground}`}>
          <div
            className={`textfield textfield-${variant} max-w-[50ch]`}
